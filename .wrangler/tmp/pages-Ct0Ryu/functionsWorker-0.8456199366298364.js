...
var init_config = __esm({
  "helpers/config.js"() {
    init_functionsRoutes_0_1477823833804186();
...
async function signJwt(payload, secret, expiresIn) {
  ...
  const expiration = Number.isFinite(expiresInSeconds) && expiresInSeconds > 0 ? Math.floor(Date.now() / 1e3) + expiresInSeconds : void 0;
  return new SignJWT(payload).setProtectedHeader({ alg: "HS256" }).setExpirationTime(expiration).sign(encoder2.encode(secret));
}
...
async function onRequestGet({ request, env }) {
  try {
    ...
    try {
      ...
      console.warn("Invalid or expired auth token:", error2?.message ?? error2);
      return jsonResponse({ success: false, message: "Invalid or expired token" }, 401);
    }
...
async function onRequestPost6({ request, env }) {
  try {
    ...
    const supabase = getSupabaseAdmin(env);
    const { data: existingUser, error: userError } = await supabase.from("users").select("id").eq("email", normalizedEmail).maybeSingle();
...
async function onRequestPost7({ request, env }) {
  ...
  try {
    ...
    const supabase = getSupabaseAdmin(env);
    const { data: existingUser, error: userError } = await supabase.from("users").select("id").eq("phone", phone).maybeSingle();
    if (userError && userError.code !== "PGRST116") {
      throw userError;
    }
    if (!existingUser) {
...
async function validateZohoPrefillToken({ env, token, allowUsed = false }) {
  if (!token) return null;
  const supabase = getSupabaseAdmin(env);
  const { data, error } = await supabase.from("zoho_prefill_tokens").select("token, user_id, expires_at, used, created_at").eq("token", token).maybeSingle();
  ...
  const expiresAt2 = typeof data.expires_at === "string" ? Number(data.expires_at) : data.expires_at;
  if (!expiresAt2 || expiresAt2 <= Date.now()) {
...
async function markZohoPrefillTokenUsed({ env, token }) {
  const supabase = getSupabaseAdmin(env);
  const { error } = await supabase.from("zoho_prefill_tokens").update({ used: true }).eq("token", token);
  if (error) throw error;
}
var ZOHO_PREFILL_TOKEN_REUSE_WINDOW_MS;
var init_zohoTokens = __esm({
  "helpers/zohoTokens.js"() {
    init_functionsRoutes_0_1477823833804186();
    init_supabase();
    ZOHO_PREFILL_TOKEN_REUSE_WINDOW_MS = 6e4;
...
async function handlePrefill(token, env) {
  ...
  try {
    ...
    if (!tokenRecord) {
      const response2 = jsonResponse({ success: false, message: "Invalid or expired token" }, 400);
      console.log("[prefill] validation result: token invalid");
      console.log("[prefill] response status:", response2.status);
      return response2;
    }
    const supabase = getSupabaseAdmin(env);
    const { data: user, error: userError } = await supabase.from("users").select("email, full_name, company_name, phone, first_name, last_name").eq("id", tokenRecord.userId).single();
    if (userError || !user) {
...
async function getPostToken(request) {
  ...
  if (trimmedContentType.includes("application/json")) {
    try {
      const body = await request.json();
      return typeof body?.token === "string" ? body.token : void 0;
    } catch (error) {
  ...
  try {
    ...
    return params.get("token") ?? void 0;
  }
}
...
var init_prefill = __esm({
  "api/forms/prefill.js"() {
    ...
    __name(getPostToken, "getPostToken");
...
// ../.wrangler/tmp/pages-Ct0Ryu/functionsRoutes-0.1477823833804186.mjs
var routes;
var init_functionsRoutes_0_1477823833804186 = __esm({
  "../.wrangler/tmp/pages-Ct0Ryu/functionsRoutes-0.1477823833804186.mjs"() {
    init_request();
    init_verify5();
